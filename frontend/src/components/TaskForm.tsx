...
export const TaskForm: React.FC<TaskFormProps> = ({
  ...
  const [status, setStatus] = useState<'Draft' | 'In Progress' | 'Done'>('Draft');
    ...
    <form onSubmit={handleSubmit} className="flex flex-col gap-6 h-full justify-between">
      <div className="flex flex-col gap-5 overflow-y-auto pr-1">
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor="taskTitle" className="text-xs font-semibold text-slate-400">
            Nama Tugas / Task
          </label>
          <input
            id="taskTitle"
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor="taskProject" className="text-xs font-semibold text-slate-400">
            Project
          </label>
          <select
            id="taskProject"
            ...
            className="bg-slate-950 border border-slate-800 text-slate-100 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-colors disabled:opacity-50"
          >
            {projects.map((p) => (
        ...
        <div className="grid grid-cols-2 gap-4">
          {/* Weight */}
          <div className="flex flex-col gap-2">
            <label htmlFor="taskWeight" className="text-xs font-semibold text-slate-400">
              Bobot (Weight)
            </label>
            <input
              id="taskWeight"
              ...
              className="bg-slate-950 border border-slate-800 text-slate-100 rounded-lg px-4 py-2.5 text-sm focus:outline-none focus:border-emerald-500 focus:ring-1 focus:ring-emerald-500 transition-colors"
            />
          </div>

          {/* Status */}
          <div className="flex flex-col gap-2">
            <label htmlFor="taskStatus" className="text-xs font-semibold text-slate-400">
              Status
            </label>
            <select
              id="taskStatus"
        ...
        <div className="flex flex-col gap-2">
          <label htmlFor="taskParent" className="text-xs font-semibold text-slate-400">
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-lg max-h-40 overflow-y-auto p-3 flex flex-col gap-2">
            {dependencyOptions.length === 0 ? (
              <span className="text-xs text-slate-600 italic">Tidak ada pilihan task lain dalam proyek ini.</span>
            ) : (
              dependencyOptions.map((t) => (
                <label key={t.id} className="flex items-center gap-2.5 text-xs text-slate-350 cursor-pointer select-none py-1 hover:text-slate-100">
                  <input
                    type="checkbox"
